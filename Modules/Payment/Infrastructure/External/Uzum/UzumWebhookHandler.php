...
final class UzumWebhookHandler
{
    ...
    private function performTransaction(Request $request): array
    {
        ...
        if ($payment->status !== PaymentStatus::PENDING) {
            return ['status' => -1, 'errorMessage' => 'Payment is not pending'];
        }
    ...
    private function verifySignature(Request $request): void
    {
        ...
        if ($username === '' || $password === '') {
            throw new InvalidSignatureException('Uzum production credentials are not configured');
        }
        ...
        if (
            ! hash_equals($username, (string) ($parts[0] ?? ''))
            || ! hash_equals($password, (string) ($parts[1] ?? ''))
        ) {
